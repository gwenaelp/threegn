...
export function BaseNode({ selected, id, _mapping }) {
  ...
  return (
    <div
      ...
      className={isInputFocused ? "nodrag" : undefined}
...
type CreateNodeComponentArgs = {
  inputs?: number[];
  outputs?: number[]
};
export function createNodeComponent(args: CreateNodeComponentArgs) {
  const { inputs = [], outputs = [] } = args;
  const mapping = { inputs, outputs };
  return (props: { selected: any; id: any; }) => {
    const { selected, id } = props;
    return (<BaseNodeMemo _mapping={mapping} selected={selected} id={id} />)
  };
}
